...
| Tab | `tab.create`, `tab.list`, `tab.get`, `tab.focus`, `tab.rename`, `tab.move`, `tab.move_to_workspace`, `tab.close` |
...
`tab.move` reorders a tab inside its workspace (`insert_index` required) and
responds with the updated `type: "tab_list"`. `tab.move_to_workspace` moves the
whole tab — panes, split layout, label, zoom, and running terminals — to
another workspace as a unit. It is a separate method so older servers reject
it as unknown instead of silently reordering:

```json
{"id":"req_tab_reorder","method":"tab.move","params":{"tab_id":"w1:t2","insert_index":0}}
{"id":"req_tab_move","method":"tab.move_to_workspace","params":{"tab_id":"w1:t2","workspace_id":"w2","focus":true}}
{"id":"req_tab_move_at","method":"tab.move_to_workspace","params":{"tab_id":"w1:t2","workspace_id":"w2","insert_index":1}}
```

`insert_index` is optional (omitted appends to the destination's tab bar). The
response is `type: "tab_move_to_workspace"` with `changed`, `previous_tab_id`,
`previous_workspace_id`, the moved `tab`, the final `insert_index`, optional
`closed_workspace_id`, a `pane_id_map` of per-pane public id reassignment, and
`target_layout`. A `workspace_id` naming the tab's own workspace reorders and
reports the same shape. The destination assigns the tab and its panes fresh
public numbers; prior public pane ids keep resolving as aliases, and agent
status follows the moved terminals. A source workspace left without tabs is
closed. Subscribers receive `tab.moved` carrying `previous_tab_id` and
`previous_workspace_id` to correlate the old identity with the new one; Herdr
does not emit fake tab close/create events for the moved tab.
...
that workspace. For a `tab.move_to_workspace` cross-workspace move, `tab.moved`
additionally carries `previous_tab_id` and `previous_workspace_id` naming the
identity the tab had in the source workspace.
