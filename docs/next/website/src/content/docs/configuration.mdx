...
Set `ui.tab_close_button = true` to show an `x` in the last cell of each tab label in the desktop tab bar while `ui.mouse_capture` is enabled. Click the `x` with the configured modifier (Alt/Option by default) to close that tab immediately, without a confirmation prompt; clicking it without the configured modifier combination activates the tab as usual. Closing the last tab of a workspace closes the workspace and still honors `ui.confirm_close`.

The modifier is configurable via `ui.tab_close_button_modifier`: `"alt"` (default), `"ctrl"`, `"shift"`, or a `+`-separated combination such as `"ctrl+shift"`. Terminal mouse reporting can only encode Shift, Alt, and Ctrl, so Cmd/Super is not accepted — it could never reach the app. Practical notes: on macOS, Ctrl-click is treated by the system as a right-click, and in iTerm2 holding Option bypasses mouse reporting (for text selection), so for the default Alt modifier the effective gesture there is **Cmd-click, which iTerm2 reports to the app as Alt-click** — disable "⌘-Click opens filename/url" (Settings → Pointer) or restrict Semantic History (Settings → Profiles → Advanced) if iTerm2 captures the click to open/download instead.
...
Set `tab_bar_wrap = true` under `[ui]` to show tabs across the available rows instead of using the default single scrollable row with `<`/`>` buttons. The tab bar expands inward from its configured edge as needed, up to half of the pane area's height, and the terminal and popup geometry shrink with it. When the row count reaches that cap, Herdr scrolls the visible row window to keep the active tab visible. The tab bar keeps the placement chosen by `tab_bar_position`, so wrapped rows grow downward from the top edge or upward from the bottom edge. The default is `false`.
...
Set `agent_panel_scope = "all"` or `agent_panel_scope = "current"` under `[ui]` to filter the Agent panel to every space or to the active space only. `"current_workspace"` is accepted as an alias for `"current"`. Default is `"all"`. Current scope sorts by attention priority unless an active Agent view supplies its own sort.
...
Herdr recognizes a Pi Agent by the name of the program running in the pane. If you run Pi through a differently named executable, such as a Pi-compatible fork or a wrapper on your `PATH`, name it here:
...
Herdr uses that name to start and detect Pi, and to replace the native `pi` executable in a Pi resume plan:
...
- **Resuming or relaunching** a Pi session uses `custom-pi` when its generated or saved launch command starts with `pi`; a saved command using another program keeps that program.

For detection the name is matched the same way as the built-in Agent names: case-insensitively, ignoring a `.exe`, `.cmd`, `.bat`, `.ps1`, or `.js` extension, and using the final path component if you give a full path. Starting an Agent uses the configured value after trimming surrounding whitespace, so a full path launches that path.

The default is `"pi"`, which changes nothing. Blank or whitespace-only values also fall back to `"pi"`. A name that already belongs to another Agent, such as `claude`, keeps that Agent, and other Agents always launch under their own executables. Reloading the config applies the new name immediately to detection and newly started Agents. A pending Pi resume reads the current setting when it starts, but substitutes it only for a launch command whose program is `pi`.
...
The value is in milliseconds and defaults to 15,000 (15 seconds), preserving the existing behavior when unset. Zero uses the default, and values above 300,000 (5 minutes) are capped at that maximum. This setting applies to the stop phase of both `herdr server stop` and `herdr server restart`; named `herdr session stop` commands continue to use the 15-second default.
...
Pi resumes substitute the current `agent.pi_program` value for a generated or saved native `pi` program when the deferred resume starts; a saved non-`pi` program is preserved.
