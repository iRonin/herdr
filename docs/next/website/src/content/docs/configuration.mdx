...
By default the agent panel shows all agents across all spaces. Set `agent_panel_scope = "current"` under `[ui]` to limit it to the active workspace; `"all"` is the default, and `"current_workspace"` remains accepted as an alias. `agent_panel_sort` remains independent: `"spaces"` groups by space, while `"priority"` orders blocked, done, working, idle, then unknown agents, breaking ties by the most recent state change. Current-space mode always uses priority ordering.

Click the agent-panel mode label to cycle through `priority`, `grouped`, and `space`. `agent_panel_modes` controls which modes appear and their order, for example `["priority", "space"]`. Unknown and duplicate entries are ignored while preserving the first occurrence; an empty or invalid-only list falls back to all three modes. A one-item list makes clicks a no-op. The mode list controls cycling only and does not change the configured or persisted `agent_panel_sort` and `agent_panel_scope` at startup.
