...
By default the agent panel shows all agents across all spaces. `agent_panel_scope` controls this: `all` (the default) shows every space's agents, and `current` limits the panel to the active workspace's agents, updating as you switch workspaces. `current_workspace` is accepted as an alias for `current`.

`agent_panel_sort` can be `spaces` or `priority`; `workspaces` is accepted as an alias for `spaces`. `spaces` is the default and keeps agents grouped by space order. `priority` sorts by attention priority: blocked, done, working, idle, then unknown. Within the same status, agents that most recently changed state appear first.

Click the agent-panel mode label to cycle through `priority`, `grouped`, and `space`. `agent_panel_modes` controls which modes appear and their order. For example, `["priority", "space"]` skips `grouped`. Unknown and duplicate entries are ignored while preserving the first occurrence; an empty or invalid-only list falls back to all three modes in the default order. A one-item list makes the click a no-op. If the current mode is omitted from a multi-item list, the next click snaps to the first configured mode.
...
Set `tab_bar_wrap = true` to wrap the tabs onto multiple rows so every tab stays visible, instead of the default single scrollable row with `<`/`>` buttons. The tab bar grows downward as needed (up to half the pane height) and the pane area below shrinks to make room. The default is `false`, which keeps the single-line scrollable tab row.

Set `tab_agent_status = true` to show the highest-attention agent state on each top tab. The indicator uses the same glyphs and colors as the agents panel: `⠋` working, `◉` blocked, `●` done-unread, `✓` done-read, and `○` unknown. It is disabled by default.
...
Choose how pane scrollbars are shown:
...
`"always"` is the default: Herdr reserves one terminal column and draws the scrollbar whenever scrollback exists. `"auto"` keeps pane text full-width and overlays the scrollbar on the rightmost text column only while the pane remains scrolled back and was scrolled recently. After about 1.2 seconds of inactivity, the overlay disappears and subsequent terminal-native selections are free of the scrollbar glyph. Use `"never"` when selections must remain clean immediately; it keeps pane text full-width and never draws the pane scrollbar. For compatibility with older configs, `true` means `"always"` and `false` means `"never"`.
...
The value is in milliseconds and defaults to 10,000 (10 seconds). Zero uses the default, and values above 300,000 (5 minutes) are capped at that maximum. This setting applies to `herdr server stop`; named `herdr session stop` commands continue to use the default timeout.
