...
- `agent_count` — recognized agent panes in the workspace, rendered as `🤖N` (including `🤖0`). It is fixed-width, so flexible labels truncate first on narrow rows. Indented worktree children keep their own count while hiding Git details. A collapsed worktree parent sums every workspace in its group.
...
Set `tab_agent_status = true` under `[ui]` to show the highest-attention agent state on each top tab. The indicator uses the same status marks as the Agent panel and the rest of the app: a filled dot for blocked and unread (red), working (yellow) and done but unread (teal); a hollow dot for blocked and read (red) and done and read (green); and a middle dot for unknown. States are distinguished by colour as well as shape, so a one-cell tab mark relies on colour to separate blocked-unread, working and done-unread. It is disabled by default and works with both single-row and wrapped tab bars.

Set `tab_agent_context = true` under `[ui]` to mirror the standalone context percentage already shown in the Agents pane for that same highest-attention agent pane. Herdr recognizes a standalone `N%·PID` or `~N%·PID` label, appends the exact `N%` or `~N%` portion after the visible tab name and before a ` Z` zoom marker, and omits the suffix when the highest-attention pane has no single valid label. It works independently of `tab_agent_status` and is disabled by default.

Choose how pane scrollbars are shown with `show_scrollbar = "always"`, `"auto"`, or `"never"` under `[ui]`. `"always"` is the default and reserves one terminal column whenever the pane is wide enough. `"auto"` keeps pane text full-width and briefly overlays the scrollbar on the rightmost text column after scrolling while the pane remains scrolled back; it disappears after about 1.2 seconds of inactivity. `"never"` keeps text full-width and never draws the pane scrollbar. Existing boolean values remain compatible: `true` means `"always"` and `false` means `"never"`.
