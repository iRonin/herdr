...
Set `tab_agent_context = true` under `[ui]` to mirror the standalone context percentage shown in the Agents pane for each tab's highest-attention agent pane. Herdr recognizes a standalone `N%·PID` or `~N%·PID` label, appends the exact `N%` or `~N%` portion after the visible tab name and before a ` Z` zoom marker, and omits the suffix when the highest-attention pane has no single valid label. It is disabled by default.
