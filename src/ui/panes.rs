...
fn stable_terminal_inner_rect(pane_inner: Rect, scrollbar_mode: ScrollbarMode) -> Rect {
    if scrollbar_mode != ScrollbarMode::Always || pane_inner.width <= 4 {
        return pane_inner;
    }
...
fn stable_scrollbar_gutter(
    ...
    let inner_rect = stable_terminal_inner_rect(pane_inner, scrollbar_mode);
...
pub(super) fn resize_tab_panes(
    ...
    if tab.zoomed {
        ...
        if let Some((terminal_id, rt)) = runtime_for_tab_pane(terminal_runtimes, tab, focused_id) {
            ...
            let inner_rect = stable_terminal_inner_rect(pane_inner, app.scrollbar_mode);
...
pub(super) fn compute_pane_infos(
    ...
    if ws.zoomed {
        ...
        if let Some(rt) = app.runtime_for_pane_in_workspace(terminal_runtimes, ws_idx, focused_id) {
            (inner_rect, scrollbar_rect) =
                stable_scrollbar_gutter(rt, pane_inner, app.scrollbar_mode);
            if resize_panes
...
mod tests {
    ...
    async fn pane_scrollbar_reserves_last_column_from_terminal_area() {
        let mut app = AppState::test_new();
        app.scrollbar_mode = crate::config::ScrollbarMode::Always;
        let mut workspace = Workspace::test_new("test");
        let root_pane = workspace.tabs[0].root_pane;
        workspace.tabs[0].runtimes.insert(
            root_pane,
    ...
    fn scrollbar_overlay_visibility_expires_and_requires_auto_scrollback() {
        ...
        app.scrollbar_mode = crate::config::ScrollbarMode::Auto;
