...
mod tests {
    ...
    fn tab_agent_status_renders_blocked_dot_only_when_enabled() {
        ...
        assert_eq!(leading_label_cell.symbol(), "1");
    ...
    #[test]
    fn tab_agent_status_working_shows_spinner_like_agents_pane() {
        let mut app = AppState::test_new();
        let ws = Workspace::test_new("test");
        let pane_id = ws.tabs[0].root_pane;
        app.workspaces = vec![ws];
        ...
        // A working agent must render the exact braille spinner the agents pane
        // shows via agent_icon, not the state_dot bullet. Frame 0 == "⠋", yellow.
        let dot_cell = &terminal.backend().buffer()
            [(app.view.tab_hit_areas[0].x + 1, app.view.tab_hit_areas[0].y)];
        assert_eq!(dot_cell.symbol(), "⠋");
        assert_eq!(dot_cell.style().fg, Some(app.palette.yellow));
    }

    #[test]
    fn tab_agent_status_uses_same_hook_only_pane_set_as_agents_panel() {
        let mut app = AppState::test_new();
        let mut ws = Workspace::test_new("test");
        ...
        hook_terminal.set_hook_authority(
            "herdr:pi".into(),
            "pi".into(),
            AgentState::Working,
            None,
        ...
        assert!(hook_terminal.detected_agent.is_none());

        let non_agent_terminal_id = app.workspaces[0].tabs[0].panes[&non_agent_pane]
            .attached_terminal_id
            .clone();
        let non_agent_terminal = app.terminals.get_mut(&non_agent_terminal_id).unwrap();
        non_agent_terminal.state = AgentState::Blocked;
        assert!(non_agent_terminal.detected_agent.is_none());
        assert!(non_agent_terminal.agent_name.is_none());
        assert!(non_agent_terminal.effective_agent_label().is_none());
        ...
        assert_eq!(
            tab_agent_state(&app.workspaces[0], &app.terminals, 0),
    ...
    #[test]
    fn wrap_mode_renders_agent_status_dot_on_lower_row() {
        let mut app = AppState::test_new();
        app.tab_bar_wrap = true;
        app.tab_agent_status = true;
        ...
        terminal
            .draw(|frame| render_tab_bar(&app, frame, app.view.tab_bar_rect))
            .unwrap();
        let dot_cell = &terminal.backend().buffer()[(status_rect.x + 1, status_rect.y)];
        assert_eq!(dot_cell.symbol(), "◉");
        assert_eq!(dot_cell.style().fg, Some(app.palette.red));
    }
}
