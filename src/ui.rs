...
pub(crate) use self::{
    ...
    sidebar::{
        agent_entry_gap, agent_entry_height_in_body, agent_panel_body_rect, agent_panel_cycle_next,
        agent_panel_entries, agent_panel_scroll_for_target, agent_panel_scroll_metrics,
        agent_panel_scrollbar_rect, agent_panel_toggle_label, agent_panel_toggle_rect_for_label,
...
fn desktop_tab_bar_and_terminal_area(
    ...
    if !hide_single_tab_bar && main_area.height > 1 {
        // Height and placement are independent axes: `tab_bar_wrap` decides how
        // many rows the bar needs, `tab_bar_position` decides which end of the
        // main area it occupies. With wrap off the height is 1, so both branches
        // reduce to upstream's exact geometry.
        ...
        match app.tab_bar_position {
            ...
            crate::config::TabBarPositionConfig::Bottom => {
                let [terminal_area, tab_bar_rect] =
                    Layout::vertical([Constraint::Min(1), Constraint::Length(tab_bar_height)])
                        .areas(main_area);
                (tab_bar_rect, terminal_area)
            }
        }
...
mod tests {
    ...
    #[test]
    fn wrapped_tab_rows_shrink_terminal_and_popup_geometry_together() {
        let mut app = crate::app::state::AppState::test_new();
        ...
        assert!(app.view.tab_bar_rect.height > 1);
        assert_eq!(app.view.terminal_area.y, app.view.tab_bar_rect.height);
        assert_eq!(
            app.view.tab_bar_rect.height + app.view.terminal_area.height,
            20
        );
        let (popup_outer, _) =
            panes::popup_pane_rects(&app, app.view.terminal_area).expect("popup rects");
        assert!(popup_outer.y >= app.view.terminal_area.y);
        assert!(
            popup_outer.y + popup_outer.height
