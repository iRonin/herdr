...
fn restore_tab(
    ...
    for id in &pane_ids {
        ...
        if let Some(plan) = pending_native_agent_restore {
            ...
            // Carry the original launch command forward so the replayed flags
            // survive repeated restarts (each restart re-persists launch_argv
            // and re-derives the selector). No respawn lifecycle here; that is
            // handoff-only.
...
mod tests {
    ...
    fn restore_plan_replays_launch_argv_when_present() {
        ...
        // harness launched as `pi-fork --session <old> --model opus` — must
        // resume through `pi-fork`, with exactly one fresh `--session`.
        ...
        assert_eq!(
            restore_plan_for_snapshot(&session, true, Some(&launch_argv))
                .unwrap()
                .argv,
            vec!["pi-fork", "--model", "opus", "--session", new_path.as_str()]
        );
    }
    ...
    async fn restore_replays_launch_argv_and_carries_it_onto_resumed_terminal() {
        ...
        // Replays the original `pi-fork` program + `--model opus`, strips the
        // stale `--session /old`, and appends exactly one fresh `--session`.
        assert_eq!(
            plan.argv,
            vec![
                "pi-fork",
