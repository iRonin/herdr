...
fn cross_area_agent_process_survives_detach_and_reattach() {
    ...
    // Blocked renders as needs-attention (◉) only for a pane you are *not*
    // looking at. A pane that blocks in the focused, active tab is acknowledged
    // on arrival and renders the hollow read mark instead, so leaving the agent
    // pane focused here would assert the read glyph while claiming to assert the
    // needs-attention one. Focus a second tab first, which is the state this
    // assertion is actually named for.
